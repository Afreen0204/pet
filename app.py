...
mydb=mysql.connector.connect(host="localhost",user="root",password="Admin",db='data1')
...
def dashboard():
    try:
        if  session.get('shelter'):
            ...
            cursor.execute('select name, species, breed, age, size, location, personality, history, image_url, shelter_id, pet_id from petprofiles where shelter_id=%s',[id])
...
def profile():
    ...
    if session.get('user'):
        ...
        cursor.close()
    return render_template('profile.html', profiles=profiles,id=id)
